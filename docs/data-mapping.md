...
| supply_charge_daily_cents | Flipped Energy Supply Charge Daily | /MyAccount/ProjectAccountData    | accounts[0].product.currentPlan.billingUnits[].periodicCharge on supply charge units                    | Convert dollars to cents when needed. Prefer daily supply-charge units.                                                                                     |
| supply_charge_daily_incl_gst_cents | Flipped Energy Supply Charge Daily (Incl GST) | /MyAccount/ProjectAccountData | accounts[0].product.currentPlan.billingUnits[].periodicCharge (incl GST unit) or explicit incl GST fields | Convert dollars to cents when needed. Prefer values explicitly marked incl GST.                                                                             |
